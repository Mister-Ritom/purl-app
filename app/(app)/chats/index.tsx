...
export default function ChatListScreen() {
  ...
  const [enrichedConvs, setEnrichedConvs] = useState<
    (Conversation & { otherUser?: UserProfile; preview: string })[]
  >([]);
      ...
      conversations.map(async (conv) => {
        ...
        if (conv.lastMessage) {
          ...
          if (conv.isGroup) {
            ...
            if (gKey)
              decrypted = decryptWithGroupKey(gKey, encryptedContent, nonce);
          } else {
            const otherUid = conv.participants.find((p) => p !== user.uid);
            if (otherUid) {
              const secret = getSharedSecretFromCache(otherUid);
              if (secret)
                decrypted = decryptMessage(secret, encryptedContent, nonce);
            }
          }
          ...
          if (decrypted) {
            if (type === "text") preview = `${prefix}${decrypted.slice(0, 40)}`;
              ...
              preview = `${prefix}📷 ${decrypted === "media" ? "Media" : decrypted}`;
