...
export default function CallsScreen() {
  ...
  const renderCall = ({ item }: { item: (Call & { otherUser?: UserProfile }) }) => {
    const isOutgoing = item.callerId === user?.uid;
    const isMissed = item.status === 'missed' && !isOutgoing;
    const name = item.otherUser?.displayName ?? item.otherUser?.username ?? 'Unknown';
    ...
    return (
      <TouchableOpacity style={styles.callRow} activeOpacity={0.7}>
        ...
        <View style={styles.callRight}>
          <Text style={styles.callTime}>{formatConversationTime(item.createdAt)}</Text>
          <Text style={styles.callTypeIcon}>{item.type === 'voice' ? '📞' : '📹'}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {calls.length === 0 ? (
        <EmptyState icon="📞" title="No calls yet" subtitle="Start a voice or video call from any conversation." />
      ) : (
        <FlatList
          data={calls}
