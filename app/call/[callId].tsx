...
export default function CallScreen() {
  ...
  // Caches the Agora token returned by initiateCall so loadCallAndJoin
  // can use it without any extra Firestore read or Cloud Function call.
  const callerTokenRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const initiate = async () => {
      try {
        ...
        const { callId: realCallId, agoraToken: callerToken } = result.data as { callId: string; agoraToken: string };
        if (active) {
          callerTokenRef.current = callerToken;
  ...
  async function loadCallAndJoin(targetCallId: string) {
    if (!user) return;
    try {
      const isCallerFlow = callId === "outgoing" && callerTokenRef.current !== null;

      let call: any;
      let token: string;

      if (isCallerFlow) {
        // ─── Caller path: zero Firestore reads, zero extra Cloud Function calls ──
        // • otherUser already set from URL params in the first useEffect
        // • token was returned inline by initiateCall and cached in callerTokenRef
        call = {
          id: targetCallId,
          callerId: user.uid,
          receiverIds: [receiverId],
        ...
        // ─── Receiver path: one Firestore read only ───────────────────────────
        // Caller name/photo are embedded in the call doc, so no user doc read needed.
        // Token was pre-generated by initiateCall and stored in agoraTokens[uid].
        ...
        call = { id: callDocSnap.id, ...callDocSnap.data() } as any;
        setActiveCall(call);

        // Use caller info embedded in the call doc (no extra user doc read)
        setOtherUser({
          uid: call.callerId,
          displayName: call.callerUsername ?? "Unknown",
          photoURL: call.callerPhotoURL ?? "",
        } as any);

        // Use pre-generated per-uid token from the call doc
        token = call.agoraTokens?.[user.uid] ?? call.agoraToken;
        if (!token) {
          // Fallback for any legacy call doc that predates agoraTokens
          try {
            const result = await httpsCallable(
              getFunctions(),
              "generateAgoraToken",
      ...
      // Mic permission was pre-requested at login — only ask for camera here
      // since it's not safe to request camera without a visible call UI.
      const isVideoCall = call.type === "video" || type === "video";
      if (isVideoCall) {
        if (Platform.OS === "ios") await request(PERMISSIONS.IOS.CAMERA);
        else await request(PERMISSIONS.ANDROID.CAMERA);
      }

      // Engine is pre-warmed at login — this is a no-op if already initialised
      setCallStatus("connecting");
      initAgoraEngine();
